...
So, we have modified our functions to return a *Promise*, now let's go back to our main code. I'm going to rewrite this closer to the previous code, so you can see both the new code and the old code side by side. So, first we need to call *getCustomer* with *1*. This returns a *Promise*, so we can *await* it, and then get a *customer* and store it in the constant. Next we need to do a *console.log*, of *customer*, then we're going to check if *customer* is gold. If that's the case, we to call *getTopMovies()*. Again, this returns a *Promise*, so we can *await* it and then get the result and store it in a constant called *movies*. After that, we're going to do another *console.log* of *movies*. And then, finally we're going to send an email. So, we call *sendEmail*, give it two arguments, *customer.email  & Top movies object*. Again this returns a *Promise*, so we *await* it. And, when we are done, we're going to do another *console.log* of *Email sent...*. Now, I told before that whenever you use *await*, your code should be inside of a function that is decorated with the *async modifier*. So here, I'm going to define a function, let's call this *notifyCustomer()*, we could call it anything, name doesn't really matter. So then, we move all the new code inside the *notifyCustomer()* function, and finally, decorate with *async*. And then we call the function at the end.

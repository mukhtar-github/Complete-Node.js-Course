...
So, we're chaining *then* and the second *Promise*. When that *Promise* is *resolved*, we're going to have our array of repositories. Now at this point we want to get the commits for the first repository, so, we call *getCommits*, and as an argument, we pass the first repository. Now, once again, the function *getCommits* also returns a *Promise*, so we can chain *then* on that *Promise*. That *Promise*, when *resolved* will eventually have the list of commits for the given repository.

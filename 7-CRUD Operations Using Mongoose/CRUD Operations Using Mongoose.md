...
Now this result is the actual *course* object that is saved in the database. So when we save this *course* in *MongoDB*, *MongoDB* is going to assign a unique identifier to this *course* object, to this *course* document. Now, with this, we can see the *id* that is assigned by *MongoDB*. So, let's log it on the console, *console.log(result)*. As I told you before, whenever you use *await*, your code should be inside an *async* function. So, I'm going to
